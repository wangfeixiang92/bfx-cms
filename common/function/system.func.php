...
function sendmail($email, $title = '', $content = '', $mailtype = 'HTML'){
	...
	qifan_base::load_sys_class('smtp', '', 0);
...
function get_memberavatar($user, $type=1, $default=true) {
	...
	if($type == 1){
		...
		$data = $member_detail->field('userpic')->join('qifancms_member b ON qifancms_member_detail.userid=b.userid')->where(array('username' => $user))->find();
	}	
 ...
 * @return array
 */
function for_route($field){
	$data = D('urlrule')->select();
	$route = array();
	foreach($data as $val){
		if(strpos($val['route'],$field)){
			$route['urlrule'] = $val['urlrule'];
			break;
		}
	}
	return $route;
