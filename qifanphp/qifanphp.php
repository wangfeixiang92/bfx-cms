...
defined('QIFANPHP_PATH') or exit('Access Denied.'); 

define('IN_QIFANPHP', true);

//QIFANPHP框架路径
define('YP_PATH', dirname(__FILE__).DIRECTORY_SEPARATOR);
//QIFANPHP框架版本信息
define('QIFANPHP_VERSION', '2.0');
//QIFANPHP应用目录
define('APP_PATH', QIFANPHP_PATH.'application'.DIRECTORY_SEPARATOR);
...
//加载公用函数库
qifan_base::load_sys_func('global');
qifan_base::load_common('function/system.func.php');
qifan_base::load_common('function/extention.func.php');
qifan_base::load_common('data/version.php'); 
defined('qifancms_SOFTNAME') or define('qifancms_SOFTNAME', base64_decode('UWlmYW5DTVPoo4Llj5jlhoXlrrnnrqHnkIbns7vnu58='));
...
class qifan_base {
	...
	private static function _load_class($classname, $path = '', $initialize = 1) {
		...
		if (empty($path)) $path = QIFANPHP_PATH.'qifanphp'.DIRECTORY_SEPARATOR.'core'.DIRECTORY_SEPARATOR.'class';
	...
	public static function load_sys_func($func) {
		if (is_file(QIFANPHP_PATH.'qifanphp'.DIRECTORY_SEPARATOR.'core'.DIRECTORY_SEPARATOR.'function'.DIRECTORY_SEPARATOR.$func.'.func.php')) {
			include QIFANPHP_PATH.'qifanphp'.DIRECTORY_SEPARATOR.'core'.DIRECTORY_SEPARATOR.'function'.DIRECTORY_SEPARATOR.$func.'.func.php';
		}
	}
	...
	public static function load_common($path, $m = '') {
		if(!$m){
			if (is_file(QIFANPHP_PATH.'common'.DIRECTORY_SEPARATOR.$path)) {
				return include QIFANPHP_PATH.'common'.DIRECTORY_SEPARATOR.$path;
			}else{
				debug::addmsg(QIFANPHP_PATH.'common'.DIRECTORY_SEPARATOR.$path.L('does_not_exist'));
			}			
		}else{
