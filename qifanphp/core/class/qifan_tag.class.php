<?php
/**
 * qifan_tag.class.php  qifancms标签类
 *
 * @author           起梵 
...
class qifan_tag{
	...
	public function centent_tag($data) {
		...
		$limit = isset($data['limit']) ? $data['limit'] : '20';
		return  D('tag_content')->field('id,tag')->join('qifancms_tag b ON qifancms_tag_content.tagid=b.id')->order('id DESC')->where('qifancms_tag_content.modelid='.$modelid.' AND qifancms_tag_content.aid='.$id)->limit($limit)->order('id ASC')->select();
	}	
	...
	public function comment_newest($data) {
		$limit = isset($data['limit']) ? $data['limit'] : '20';
		return D('comment')->field('userid,username,userpic,inputtime,content,title,url,catid,modelid')->join('qifancms_comment_data b ON qifancms_comment.commentid=b.commentid')->where('`status`=1')->order('id DESC')->limit($limit)->select();
	}
