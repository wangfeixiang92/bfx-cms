<?php
/**
 * qifan_tpl.class.php  qifan_tpl模板解析类
 *
 * @author           起梵 
...
class qifan_tpl {
	...
	public function tpl_replace($str) {
		...
		$str = preg_replace_callback("/".$this->template_tag_left."m:(\w+)\s+([^}]+)".$this->template_tag_right."/i", array($this, 'qifan_tag_callback'), $str);

		$str = "<?php defined('IN_QIFANPHP') or exit('No permission resources.'); ?>" . $str;
		return $str;
	}	

	
	public static function qifan_tag_callback($matches) {
		return self::qifan_tag($matches[1],$matches[2], $matches[0]);;
	}
	 ...
	 * 解析标签
	 */	
	public static function qifan_tag($action, $data, $html) {
		...
		$str = '$tag = qifan_base::load_sys_class(\'qifan_tag\');';
