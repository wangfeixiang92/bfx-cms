...
function thumb($imgurl, $width = 300, $height = 200 ,$autocut = 0, $smallpic = 'nopic.jpg') {
	...
	$upload_path = QIFANPHP_PATH.C('upload_file').'/';
	...
	if(!is_object($image)) {
		qifan_base::load_sys_class('image','','0');
		$image = new image(1);
	}
...
function set_cookie($name, $value = '', $time = 0) {
	...
	$value = is_array($value) ? 'in_qifanphp'.string_auth(json_encode($value),'ENCODE',md5(QIFANPHP_PATH.C('db_pwd'))) : string_auth($value,'ENCODE',md5(QIFANPHP_PATH.C('db_pwd')));
...
function get_cookie($name = '', $default = '') {
	...
	if(isset($_COOKIE[$name])){
		if(strpos($_COOKIE[$name],'in_qifanphp')===0){
			$_COOKIE[$name] = substr($_COOKIE[$name],9);
			return json_decode(MAGIC_QUOTES_GPC?stripslashes(string_auth($_COOKIE[$name],'DECODE',md5(QIFANPHP_PATH.C('db_pwd')))):string_auth($_COOKIE[$name],'DECODE',md5(QIFANPHP_PATH.C('db_pwd'))), true);
        }
		return string_auth(safe_replace($_COOKIE[$name]),'DECODE',md5(QIFANPHP_PATH.C('db_pwd')));
	}else{
		return $default;
	}	
}
...
function M($classname){
	return qifan_base::load_model($classname);
}
...
function D($tabname){
	...
	qifan_base::load_sys_class('db_factory', '', 0);
...
function C($key = '', $default = '') {
	...
	$path = QIFANPHP_PATH.'common'.DIRECTORY_SEPARATOR.'config'.DIRECTORY_SEPARATOR.'config.php';
	if (is_file($path)) { 
...
function setcache($name, $data, $timeout=0) {
	qifan_base::load_sys_class('cache_factory','',0);
...
function template($module = '', $template = 'index'){
	...
	$template_c = QIFANPHP_PATH.'cache'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR;
	...
	if(!is_file($tplfile)) {
		showmsg(str_replace(QIFANPHP_PATH, '', $tplfile).L('template_does_not_exist'),'stop');			                      
	}	
	if(!is_dir(QIFANPHP_PATH.'cache'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR)){
		@mkdir(QIFANPHP_PATH.'cache'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR, 0777, true);
	}	
	$template_c = $template_c.$template.'.tpl.php'; 		
	if(!is_file($template_c) || filemtime($template_c) < filemtime($tplfile)) {
		$qifan_tpl = qifan_base::load_sys_class('qifan_tpl');
		$compile = $qifan_tpl->tpl_replace(@file_get_contents($tplfile));
...
function write_log($message, $filename = '', $path = '') {
	...
	if(!$path) $path = QIFANPHP_PATH.'cache';
