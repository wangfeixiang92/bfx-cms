...
class update_urls extends common {
	 ...
	 */
 	public function update_content_url() {
		...
		if(!$modelid){
			...
			foreach($r as $val){
				$url = $this->get_url($val['catid'], $val['id']);
	...
	private function get_url($catid, $id){
		...
		$field = 'show/catid/$1/id/$2';
		$route = for_route($field);
		$catinfo = get_category($catid);
		if($url_rule == '1'){//伪静态模式
			if(!empty($route)){//自定义规则
				if(substr_count($route['urlrule'], '(\d+)') == 0){
					$url = SITE_URL.$route['urlrule'];
				}elseif(substr_count($route['urlrule'], '(\d+)') == 1){
					$url = SITE_URL.explode('(\d+)',$route['urlrule'])[0].$id.explode('(\d+)',$route['urlrule'])[1];
				}else{
					$url = SITE_URL.explode('(\d+)',$route['urlrule'])[0].$catid.explode('(\d+)',$route['urlrule'])[1].$id.explode('(\d+)',$route['urlrule'])[2];
				}
			}else{
				$url = URL_MODEL == 1 ? SITE_URL.'index.php?s=/'.$catinfo['catdir'].'/'.$id.C('url_html_suffix') : SITE_URL.$catinfo['catdir'].'/'.$id.C('url_html_suffix');
			}
		}elseif($url_rule == '2'){
			$link = U('mobile/index/show',array('catid'=>$catid,'id'=>$id));
			$url = durl($link);
		}elseif($url_rule == '3'){
			if(!empty($route)){//自定义规则
				if(substr_count($route['urlrule'], '(\d+)') == 0){
					$link = SITE_URL.$route['urlrule'];
				}elseif(substr_count($route['urlrule'], '(\d+)') == 1){
					$link = SITE_URL.explode('(\d+)',$route['urlrule'])[0].$id.explode('(\d+)',$route['urlrule'])[1];
				}else{
					$link = SITE_URL.explode('(\d+)',$route['urlrule'])[0].$catid.explode('(\d+)',$route['urlrule'])[1].$id.explode('(\d+)',$route['urlrule'])[2];
				}
			}else{
				$link = URL_MODEL == 1 ? SITE_URL.'index.php?s=/'.$catinfo['catdir'].'/'.$id.C('url_html_suffix') : SITE_URL.$catinfo['catdir'].'/'.$id.C('url_html_suffix');
			}
			$url = durl($link);
		}else{
...
function durl($link){
	...
	$data = file_get_contents("http://api.t.sina.com.cn/short_url/shorten.json?source=".$source."&url_long=".$link);
	$data1 = trim($data,'[');
	$data2 = trim($data1,']');
