...
qifan_base::load_controller('common', 'admin', 0);
...
class category extends common {
	...
	public function init() {
		...
		$tree = qifan_base::load_sys_class('tree');
		$tree->icon = array('&nbsp;&nbsp;&nbsp;│ ','&nbsp;&nbsp;&nbsp;├─ ','&nbsp;&nbsp;&nbsp;└─ ');
		$tree->nbsp = '&nbsp;&nbsp;&nbsp;';
		$data = $this->db->field('catid AS id,catname AS name,show_template,parentid,type,modelid,listorder,member_publish,pclink,display')->order('listorder ASC,catid ASC')->select();
		$array = array();
		foreach($data as $v) {
			if($v['type']=="0"){ 
			...
			$v['cattype'] = $v['type']=="0" ? '系统栏目' : '<span style="color:green">自定义栏目</span>' ;
			$v['catmodel'] = $v['modelid'] ? $modelarr[$v['modelid']] : '';
			$v['display'] = $v['display'] ? '是' : '<span style="color:red">否</span>';
			$v['show_template'] = $show_temp[$v['show_template']].'('.$v['show_template'].')';
			$v['member_publish'] = $v['member_publish'] ? '<span style="color:red">是</span>' : '否';
			if($v['type'] == '0'){
				$v['string'] = '<a title="编辑栏目" href="javascript:;" onclick="qifan_open(\'编辑栏目\',\''.U('edit',array('type'=>$v['type'],'catid'=>$v['id'])).'\',800,500)" class="btn-mini btn-success ml-5" style="text-decoration:none">编辑</a>
				<span title="删除" class="btn-mini btn-disabled ml-5" style="text-decoration:none">删除</span>';
			}else{
				$v['string'] = '<a title="编辑栏目" href="javascript:;" onclick="qifan_open(\'编辑栏目\',\''.U('edit',array('type'=>$v['type'],'catid'=>$v['id'])).'\',800,500)" class="btn-mini btn-success ml-5" style="text-decoration:none">编辑</a> 
			<a title="删除" href="javascript:;" onclick="qifan_del(\''.U('delete',array('type'=>$v['type'],'catid'=>$v['id'])).'\')" class="btn-mini btn-warning ml-5" style="text-decoration:none">删除</a>';
	...
	public function edit() {				
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($_POST['arrparentid']!=$_POST['cpath']){
				...
				$this->db->query("UPDATE qifancms_category SET arrparentid=REPLACE(arrparentid, '{$_POST['cpath']}','{$_POST['arrparentid']}') WHERE arrparentid like '{$cpath}%'"); 
			}
	...
	*/
	public function page_content() {
		...
		qifan_base::load_sys_class('form','',0);
