...
class index{
	...
	public function init(){	
		...
		$search_data = $db->field('title,description,inputtime,updatetime,click,thumb,nickname,url,catid,id')->where($where)->order('id DESC')->limit($page->limit())->select();
	...
	public function tag(){	
		...
		$page = new page($total, 5);
		$search_data = $tag_content->field('title,description,inputtime,updatetime,click,thumb,nickname,url,b.catid')->join('`qifancms_article` b ON qifancms_tag_content.aid=b.id')->where(array('modelid'=>$modelid,'tagid'=>$id))->order('updatetime DESC')->limit($page->limit())->select();
