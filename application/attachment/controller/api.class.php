...
qifan_base::load_sys_class('upload','',0);
...
class api{
	...
	public function img_cropper(){
		$qifancms_path = SITE_PATH == '/' ? QIFANPHP_PATH : str_replace(SITE_PATH, '', str_replace('\\', '/', QIFANPHP_PATH));
		if(isset($_POST['filepath'])){
			$x = $_POST['x'];
			$y = $_POST['y'];
			$w = $_POST['w'];
			$h = $_POST['h'];
			$image = qifan_base::load_sys_class('image');
			$filepath = SITE_PATH.C('upload_file').'/'.date('Ym/d/');
			$filename = date("ymdhis").rand(100,999);
			$filetype = fileext($_POST['filepath']);
			$fileinfo = $image->crop($qifancms_path.$_POST['filepath'], $qifancms_path.$filepath.$filename.'.'.$filetype, $w, $h, $x, $y);
			if($fileinfo){
		...
		if(strpos($filepath, 'ttp:') || !is_file($qifancms_path.$filepath)) showmsg('请选择本地已存在的图像！', 'stop');
